...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($titulo === '' || $precio <= 0 || $stock < 1 || $descripcion === '') {
    ...
    if ($imagenes_subidas === 0) {
      ...
      try {
        // 1. Insertar el producto y obtener su ID (mantenemos el campo imagen para compatibilidad)
        $imagen_principal = $imagenes_urls[0]; // La primera imagen como principal
        ...
        $result_producto = pg_query_params($conexion, $query_producto, [
          $titulo, $precio, $descripcion, $stock, $categoria, $fecha_publicacion, $imagen_principal
        ]);
