...
DO $$
BEGIN
  -- Agrega constraint de tipo si no existe (compatibilidad con PostgreSQL < 16)
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'cupon_tipo_chk'
      AND conrelid = 'cupon'::regclass
  ) THEN
    ALTER TABLE Cupon
      ADD CONSTRAINT cupon_tipo_chk
      CHECK (tipo IN ('porcentaje','fijo'));
  END IF;

  -- Agrega unique por (id_producto, codigo) si no existe
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'cupon_codigo_unico'
      AND conrelid = 'cupon'::regclass
  ) THEN
    ALTER TABLE Cupon
      ADD CONSTRAINT cupon_codigo_unico
...
-- Categorías predeterminadas (idempotente)
...
-- Agregar columna id_categoria a Producto y FK (idempotente)
ALTER TABLE Producto
  ADD COLUMN IF NOT EXISTS id_categoria INT NULL;
